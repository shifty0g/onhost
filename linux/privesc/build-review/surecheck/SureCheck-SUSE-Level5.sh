...
if [ "$COLLECT" = 1 ]; then
	...
	echo "----- SURECHECK-PLATFORM: UNIX/Linux/SUSE -----" >> $OUTPUT
...
collect 'PLATFORM-CAT-ETC-SUSE-RELEASE' 'cat /etc/SuSE-release'
...
collect 'grub.conf' 'cat /boot/grub/menu.lst'
...
collect 'ls-grub.conf' 'ls -lnLd /boot/grub/menu.lst'
collect 'lsmod' 'lsmod'
...
if [ "$COLLECT" = 1 ]; then echo; fi
collect '/etc/SuSE-release' 'cat /etc/SuSE-release'
collect '/etc/rc.status' 'cat /etc/rc.status'
collect 'chkconfig' 'chkconfig -A -l | cat'
collect 'ntpd-options' 'cat /etc/sysconfig/ntp'
collect 'password-properties' 'passwd -Sa'
...
collect 'rpm-verify' 'rpm -Va --nomtime --nosize --nomd5 --nolinkto'
if [ "$COLLECT" = 1 ]; then echo; fi
